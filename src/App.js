...
const Apikey = '191a3ec880b1b40f19f387c8e45c66dc';
function App() {
  const [data, setData] = useState(null);
  const [location, setLoaction] = useState('Noida');
      ...
      <div className='w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-col items-center justify-center px-4 lg:px-0'>
        ...
        <form className={`${animate ? 'animate-shake' : 'animate-none'} h-16 bg-black/30 w-full max-w-[450px] rounded-full backdrop-blur-[32px] mb-8`}>
          <div className='h-full relative flex items-center justify-between p-2'>
            <input onChange={(e) => handleInput(e)} type='text' placeholder='Search by city or coutry' className='flex-1 h bg-transparent outline-none placeholder:text-white text-white text-[15px] font-light pl-6 h-full' />
            <button onClick={(e) => handleSubmit(e)} className='bg-[#1ab8ed] w-20 h-12 rounded-full flex justify-center items-center transition'><IoMdSearch className='text-2xl text-white' /></button>
          </div>
        </form>
              ...
              <div className='max-w-[378px] max-auto flex flex-col gap-y-6'>
                <div className='flex justify-between'>
                  ...
                  <div className='flex justify-around items-center gap-x-2'>
                    ...
                    <div className='flex'>Feels Like <span className='flex ml-2'>{data.main.feels_like} <TbTemperatureCelsius /></span></div>
                  </div>
                </div>
                <div className='flex justify-between'>
                  <div className='flex items-center gap-x-2'>
                  ...
                  <div className='flex items-center gap-x-2'>
                    ...
                    <div className='flex'>Wind <span className='flex ml-2'>{data.wind.speed} m/s <TbTemperatureCelsius /></span></div>
                  </div>
                </div>
              </div>
